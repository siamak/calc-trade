...
function GitHubStarsButton({
	...
	return (
		...
		>
			...
			<span>Star⚠ The "middleware" file convention is deprecated. Please use "proxy" instead. Learn more: https://nextjs.org/docs/messages/middleware-to-proxy
  Creating an optimized production build ...</span>
			<div className="relative inline-flex size-[18px] shrink-0">
				...
				/>
				<Star
					className="absolute top-0 left-0 text-amber-600 fill-amber-600"
					aria-hidden="true"
					style={{
						clipPath: `inset(${100 - (isCompleted ? fillPercentage : fillPercentage - 10)
							}% 0 0 0)`,
					}}
				/>
				<AnimatePresence>
					{displayParticles && (
						<>
							...
							{[...Array(6)].map((_, i) => (
								<motion.div
									key={i}
									className="absolute w-1 h-1 rounded-full bg-amber-600"
